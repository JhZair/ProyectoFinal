...
class Jugador:
    def __init__(self, color, rectan, aura, velocidad=5, fuerza_salto=10, gravedad=0.7):
        ...
        # Animaciones
        self.animaciones = {
            "quieto": [pygame.image.load("assets/anims/anim1.png"), pygame.image.load("assets/anims/anim1.png")],
            "caminando": [pygame.image.load("assets/anims/anim1.png"), pygame.image.load("assets/anims/anim1.png")],
            "saltando": [pygame.image.load("assets/anims/anim1.png")],
            "atacando": [pygame.image.load("assets/anims/anim1.png"), pygame.image.load("assets/anims/anim1.png")]
        }
        self.estado = "quieto"
        self.frame_actual = 0
        self.tiempo_animacion = pygame.time.get_ticks()
        self.tiempo_entre_frames = 100  # tiempo en milisegundos entre cada frame de la animación
    ...
    def actualizar(self, teclas, izq, derecha, salto, defensa, otro_jugador):
        ...
        if pygame.time.get_ticks() - self.tiempo_animacion > self.tiempo_entre_frames:
            self.tiempo_animacion = pygame.time.get_ticks()
            self.frame_actual = (self.frame_actual + 1) % len(self.animaciones[self.estado])
...
class Juego:
    ...
    def comprobar_colisiones_y_ataques(self, teclas, ataque_j1, ataque_j2, ataque_especial_j1, ataque_especial_j2):
        ...
        if self.jugador1.aura.colliderect(self.jugador2.rectan) and teclas[ataque_j1] and tiempo_actual > self.jugador1.cooldown and not self.jugador1.defendiendo:
            ...
            if self.jugador2.defendiendo:
                ...
                if self.jugador2.bajar_salud(20):
                    print("Jugador 2 ha sido derrotado")
                    return True
        ...
        if self.jugador2.aura.colliderect(self.jugador1.rectan) and teclas[ataque_j2] and tiempo_actual > self.jugador2.cooldown and not self.jugador2.defendiendo:
            ...
            if self.jugador1.defendiendo:
                ...
                if self.jugador1.bajar_salud(20):
                    print("Jugador 1 ha sido derrotado")
                    return True

        if self.jugador1.aura.colliderect(self.jugador2.rectan) and teclas[ataque_especial_j1] and tiempo_actual > self.jugador1.cooldown and self.jugador1.ataque_especial == 200:
            self.jugador1.cooldown = tiempo_actual + 2000
            self.jugador1.ataque_especial = 0
            if self.jugador2.bajar_salud(70):
                print("Jugador 2 ha sido derrotado")
                return True

        if self.jugador2.aura.colliderect(self.jugador1.rectan) and teclas[ataque_especial_j2] and tiempo_actual > self.jugador2.cooldown and self.jugador2.ataque_especial == 200:
            self.jugador2.cooldown = tiempo_actual + 2000
            self.jugador2.ataque_especial = 0
            if self.jugador1.bajar_salud(70):
                print("Jugador 1 ha sido derrotado")
                return True
    ...
    def dibujar_tiempo_restante(self, tiempo_restante):
        ...
        texto_tiempo = fuente.render(f"{minutos:02}:{segundos:02}", True, negro)
    ...
    def determinar_ganador(self):
        ...
        elif self.jugador1.salud < self.jugador2.salud:
    ...
    def ejecutar(self):
        while self.ejecutando:
            ...
            # Dibujar los jugadores y la plataforma
            self.jugador1.dibujar()
            self.jugador2.dibujar()
